...
class EnglishLearningAgentDependencies(BaseModel):
    ...
    flashcards: list[FlashCard] = Field(default_factory=lambda: [
        FlashCard(
            word="scrumptious",
...
def create_english_learning_agent(openai_api_key: str, model: str) -> Agent:
    ...
    def fetch_flashcard(context: RunContext[EnglishLearningAgentDependencies]) -> FlashCard:
        return context.deps.flashcards[
            context.deps.current_flashcard_index
        ]
...
async def run_english_learning_agent(text: str, env_config: EnvConfig) -> str:
    global agent
    if agent is None:
...
async def run_streaming_english_learning_agent(
    ...
    **kwargs
    ...
    deps = EnglishLearningAgentDependencies()
    async with agent.iter(text, deps=deps) as run:
                    ...
                    async for event in request_stream:
                        if isinstance(event, PartDeltaEvent) and isinstance(event.delta, TextPartDelta):
                            ...
                            if chunk_size is None:
                                await callback(event.delta.content_delta, *args, **kwargs)
                                continue
